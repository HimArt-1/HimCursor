    ...
    @if (appLock.isLocked()) {
      <div class="fixed inset-0 z-[9999] bg-[#fdfaf6]/95 backdrop-blur-xl flex flex-col items-center justify-center animate-in fade-in duration-300">
        ...
        <div class="w-32 h-32 bg-white rounded-[40px] shadow-2xl flex items-center justify-center mb-10 relative overflow-hidden group">
            ...
            <div [innerHTML]="lockIconHtml" class="w-16 h-16 text-[#7A4E2D] animate-pulse"></div>
        </div>
        ...
        >
            <div [innerHTML]="fingerprintIconHtml" class="w-7 h-7 fill-white group-hover:rotate-12 transition-transform"></div>
...
export class LockScreenComponent {
    ...
    lockIconHtml = this.sanitizer.bypassSecurityTrustHtml(this.lockIcon);
    fingerprintIconHtml = this.sanitizer.bypassSecurityTrustHtml(this.fingerprintIcon);
