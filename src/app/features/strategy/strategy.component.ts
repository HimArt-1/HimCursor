    ...
    <div class="space-y-8 animate-fade-in pb-10">
      ...
      <div class="bg-white dark:bg-wushai-black dark:border-wushai-olive border border-wushai-sand rounded-2xl p-6 shadow-sm">
         ...
         <div class="bg-gray-50 dark:bg-gray-900 p-6 rounded-xl font-mono text-sm border border-gray-200 dark:border-gray-800 text-gray-700 dark:text-gray-300 whitespace-pre-wrap leading-relaxed">
            ...
            <span class="text-gray-500 block mt-4 italic">Generated by Washa Control Strategy Engine.</span>
         </div>
...
export class StrategyComponent {
   ...
   constructor() {
      // Dynamic rolling 6-month window
      const now = new Date();
      const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
      for (let i = -1; i < 5; i++) {
         const d = new Date(now.getFullYear(), now.getMonth() + i, 1);
         this.timelineMonths.push(monthNames[d.getMonth()]);
      }
   }
   ...
   // Calculate left position % based on dynamic 6-month window
   getDatePosition(dateStr: string): number {
      const now = new Date();
      const start = new Date(now.getFullYear(), now.getMonth() - 1, 1).getTime();
      const end = new Date(now.getFullYear(), now.getMonth() + 5, 0).getTime();
